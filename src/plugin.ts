...
export const meta = createBuilder("meta", "metaExtracted")
  ...
  .handler(async (p, o) => {
    ...
    head = {
      ...head,
      ...Object.fromEntries(c?.headProps.map(
        ( p) => [p, frontmatter[p as string]],
      )),
    };
    ...
    const routeMeta: RouteConfig = {
      ...
      ...(c.routePath ? { path: valueOrCallback(c.routePath, [fileName, frontmatter]) as string } : {}),
    ...
    if (hasRouteConfig || o.queryParameters) {
      ...
      if (o.queryParameters) {
        router.push("const route = useRoute()", "const queryParams: Record<string, string|boolean|number> = route.query");
      }

      if (hasRouteConfig) {
        router.push("const router = useRouter()");
        if (routeMeta.name) {router.push(`router.currentRoute.value.name = ${JSON.stringify(routeMeta.name)}`);};

        if (routeMeta.path) {router.push(`router.currentRoute.value.path = ${JSON.stringify(routeMeta.path)}`);};

        if (routeMeta.meta) {
          router.push("router.currentRoute.value.meta = {", "  ...router.currentRoute.value.meta,");
